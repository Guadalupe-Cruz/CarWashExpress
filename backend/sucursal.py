...
# Función para recuperar una sucursal del histórico y agregarla nuevamente a la tabla sucursal
def recuperar_sucursal(id_sucursal, nombre_sucursal, direccion):
    connection = get_db_connection()
    cursor = connection.cursor()
    
    # Inserta la sucursal de nuevo en la tabla 'sucursal'
...
def get_historico_sucursales():
    connection = get_db_connection()
    cursor = connection.cursor()
    cursor.execute('SELECT id_sucursal, nombre_sucursal, direccion, fecha_borrado FROM sucursales_historicos')
    historico = cursor.fetchall()
    connection.close()
    return historico

# Función para eliminar una sucursal (mueve la sucursal al histórico)
def delete_sucursal(id_sucursal):
    connection = get_db_connection()
    cursor = connection.cursor()
    
    # Recuperamos la sucursal antes de eliminarla
