...
def update_usuario(id_usuario, nombre, apellido1, apellido2, correo, contrasena, telefono, direccion, puesto, id_rol, id_sucursal):
    ...
    try:
        cursor.execute("""
            UPDATE usuarios 
            SET nombre_usuario = %s, apellido_pt = %s, apellido_mt = %s, correo = %s, contrasena = %s, telefono = %s, direccion = %s, puesto = %s, id_rol = %s, id_sucursal = %s
            WHERE id_usuario = %s
        """, (nombre, apellido1, apellido2, correo, contrasena, telefono, direccion, puesto, id_rol, id_sucursal, id_usuario))
        conn.commit()
    except Exception as e:
