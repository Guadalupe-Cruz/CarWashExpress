...
# Función para recuperar el insumo y agregarlo nuevamente a la tabla de insumos
def recuperar_insumo(id_insumo, nombre_insumo, inventario, unidades, cantidad_minima):
    connection = get_db_connection()
    cursor = connection.cursor()

    # Inserta el insumo de nuevo en la tabla 'insumos' con la fecha actual
    cursor.execute('''
...
def get_historico_insumos():
    connection = get_db_connection()
    cursor = connection.cursor()
    cursor.execute('SELECT id_insumo, nombre_insumo, inventario, unidades, cantidad_minima, fecha_suministro, fecha_borrado FROM insumos_historicos')
...
def delete_insumo(id_insumo):
    ...
    cursor.execute('SELECT id_insumo, nombre_insumo, inventario, unidades, cantidad_minima FROM insumos WHERE id_insumo = %s', (id_insumo,))
    insumo = cursor.fetchone()

    if insumo:
        # Insertamos el insumo en el histórico con la fecha actual
        cursor.execute('''
