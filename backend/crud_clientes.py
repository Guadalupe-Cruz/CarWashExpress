...
# Función para eliminar un cliente (moverlo al histórico) y transferir los pagos a historico
def delete_cliente(id_cliente):
    connection = get_db_connection()
    cursor = connection.cursor()
    ...
    if cliente:
        # Mover los pagos del cliente al histórico
        cursor.execute('SELECT id_pago, monto_pagado, metodo_pago, fecha_pago, id_cliente, id_lavado FROM pagos WHERE id_cliente = %s', (id_cliente,))
        ...
        for pago in pagos:
            cursor.execute('INSERT INTO pagos_historicos (id_pago, monto_pagado, metodo_pago, fecha_pago, id_cliente, id_lavado, fecha_borrado) VALUES (%s, %s, %s, %s, %s, %s, NOW())',
                           (pago[0], pago[1], pago[2], pago[3], pago[4], id_cliente))

        # Eliminar los pagos del cliente de la tabla 'pagos'
