...
def get_historico_clientes():
    connection = get_db_connection()
    cursor = connection.cursor()
    cursor.execute('SELECT id_cliente, nombre_cliente, apellido_pt, apellido_mt, correo, telefono, fecha_expiracion_membresia, fecha_borrado FROM clientes_historicos')
...
def delete_cliente(id_cliente):
    ...
    cursor.execute('SELECT id_cliente, nombre_cliente, apellido_pt, apellido_mt, correo, telefono FROM clientes WHERE id_cliente = %s', (id_cliente,))
