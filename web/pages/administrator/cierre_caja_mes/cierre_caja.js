...
function loadClients(page = 1) {
    ...
    eel.cierre_caja_mes(page)(function (data) {
        ...
        data.cierre_cajas.forEach(cierre => {
            let row = `<tr>
                <td>${cierre.id_usuario}</td>
                <td><span class="status pending">${cierre.nombre_del_usuario}</span></td>
                <td><span class="status delivered">${cierre.total_ventas}</span></td>
                <td><span class="status shipped">${cierre.mes_nombre}</span></td>
            </tr>`;
