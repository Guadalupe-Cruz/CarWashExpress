...
function loadClients(page = 1) {
    ...
    eel.cierre_caja(page)(function (data) {
        ...
        data.cierre_cajas.forEach(cierre => {
            let row = `<tr>
                <td>${cierre.id_cierre_caja}</td>
                <td><span class="status pending">${cierre.nombre_usuario}</span></td>
                <td><span class="status delivered">${cierre.monto_total}</span></td>
                <td><span class="status shipped">${cierre.fecha_cierre}</span></td>
            </tr>`;
